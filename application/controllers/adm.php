...
class Adm extends CI_Controller {
	...
	public function m_ujian() {
		...
		if ($uri3 == "det") {
			...
			if ($this->input->post('id') != 0) {
				...
				$tgl_mulai = $this->input->post('tgl_mulai') . " 00:00:00";
				$terlambat = $this->input->post('terlambat') . " 00:00:00";
				$jenis = $this->input->post('acak');
				...
				for ($i=0; $i < $jml_mapel; $i++) { 
					$mapel_tes_id = $this->input->post('mapel_tes_id_'.$i);
					$mapel_id = $this->input->post('mapel_'.$i);
					$jml_soal = $this->input->post('jumlah_soal_'.$i);
					$waktu = $this->input->post('waktu_'.$i);

					$query = "UPDATE mapel_tes SET 
								mapel_id = ".$mapel_id.",
								jml_soal = ".$jml_soal.",
								waktu = ".$waktu."
								WHERE id = ".$mapel_tes_id;
					$this->db->query($query);
				}
				$pesan = "Berhasil Mengedit Ujian";
				...
				$tes = $this->db->query("INSERT INTO tr_guru_tes VALUES (
					NULL,
					'".$id_guru."',
					'".$nama_ujian."',
					'".$jenis."',
			...
			$ret_arr['caption']	= $ket." sukses";
			j($ret_arr);
			$this->session->set_flashdata('k', '<div class="alert alert-info">'.$pesan.'</div>');
			...
			exit();
		        ...
		        $d_total_row = $this->db->query("SELECT id
		        	FROM tr_guru_tes
		        	WHERE id_guru = '".$a['sess_konid']."' 
                    AND (nama_ujian LIKE '%".$search['value']."%' 
					)")->num_rows();
		    	
		    	//echo $this->db->last_query();

		        $q_datanya = $this->db->query("SELECT *,
										(SELECT SUM(waktu) FROM mapel_tes WHERE guru_tes_id=tr_guru_tes.id) waktu,
										(SELECT SUM(jml_soal) FROM mapel_tes WHERE guru_tes_id=tr_guru_tes.id) jml_soal
												FROM tr_guru_tes 
									        	WHERE id_guru = '".$a['sess_konid']."'
							                    AND (nama_ujian LIKE '%".$search['value']."%') 
		                                        ORDER BY id DESC LIMIT ".$start.", ".$length."")->result_array();
		        $data = array();
		        $no = ($start+1);

		        foreach ($q_datanya as $d) {
		        	$jml_mapel = $this->db->query("SELECT id FROM mapel_tes WHERE guru_tes_id=".$d['id'])->num_rows();
		        	$jh = "SELECT id FROM mapel_tes WHERE guru_tes_id=".$d['id'];
		            ...
		            $data_ok[3] = tjs($d['tgl_mulai'],"s");
		            $data_ok[4] = $jenis_soal;
		            $data_ok[5] = '
		            	<div class="btn-group">
                          <a href="'.base_url().'adm/m_ujian/edit/'.$d['id'].'?jml_mapel='.$jml_mapel.'" class="btn btn-info btn-xs"><i class="glyphicon glyphicon-pencil" style="margin-left: 0px; color: #fff"></i> &nbsp;&nbsp;Edit</a>
			...
			if ($uri4 == 0) {
				$a['d'] = array(
					...
					"tgl_mulai"=>"",
					"terlambat"=>"",
					"jml_mapel"=>$this->input->get('jml_mapel')
				);
				for ($i=0; $i < $a['d']['jml_mapel']; $i++) { 
					$a['d']['arr_mapel'][] = [
						'id' => 0,
						'mapel_id' => 0,
						'guru_tes_id' => 0,
	...
	public function h_ujian() {
		...
		if ($uri3 == "det") {
			$a['detil_tes'] = $this->db->query("SELECT m_guru.nama AS nama_guru, tr_guru_tes.*, (SELECT SUM(waktu) FROM mapel_tes WHERE guru_tes_id=tr_guru_tes.id) waktu, (SELECT SUM(jml_soal) FROM mapel_tes WHERE guru_tes_id=tr_guru_tes.id) jml_soal 
												FROM tr_guru_tes 
	        ...
	        $d_total_row = $this->db->query("SELECT a.id,
										(SELECT SUM(waktu) FROM mapel_tes WHERE guru_tes_id=a.id) waktu,
										(SELECT SUM(jml_soal) FROM mapel_tes WHERE guru_tes_id=a.id) jml_soal  FROM tr_guru_tes a
	        	INNER JOIN m_guru c ON a.id_guru = c.id
	            WHERE (a.nama_ujian LIKE '%".$search['value']."%' OR c.nama LIKE '%".$search['value']."%') ".$wh_1."")->num_rows();
	    	//echo $this->db->last_query();

	        $q_datanya = $this->db->query("SELECT a.*, c.nama AS nama_guru,
										(SELECT SUM(waktu) FROM mapel_tes WHERE guru_tes_id=a.id) waktu,
										(SELECT SUM(jml_soal) FROM mapel_tes WHERE guru_tes_id=a.id) jml_soal   FROM tr_guru_tes a
	        	INNER JOIN m_guru c ON a.id_guru = c.id
	            WHERE (a.nama_ujian LIKE '%".$search['value']."%' OR c.nama LIKE '%".$search['value']."%') ".$wh_1." ORDER BY a.id DESC LIMIT ".$start.", ".$length."")->result_array();
	        ...
	        foreach ($q_datanya as $d) {
	            ...
	            $data_ok[3] = $d['jml_soal'];
	            $data_ok[4] = $d['waktu'];
	            $data_ok[5] = '<a href="'.base_url().'adm/h_ujian/det/'.$d['id'].'" class="btn btn-info btn-xs"><i class="glyphicon glyphicon-search" style="margin-left: 0px; color: #fff"></i> &nbsp;&nbsp;Lihat Hasil</a>
                         ';
	...
	public function ikut_ujian() {
		...
		if ($uri3 == "simpan_satu") {
			...
			$a['du'] = $this->db->query("SELECT a.id, a.tgl_mulai, a.terlambat, 
										...
										(SELECT SUM(waktu) FROM mapel_tes WHERE guru_tes_id=a.id) waktu,
										(SELECT SUM(jml_soal) FROM mapel_tes WHERE guru_tes_id=a.id) jml_soal 
										FROM tr_guru_tes a 
										INNER JOIN m_guru b ON a.id_guru = b.id
			...
			if ($cek_sdh_selesai < 1) {
				...
				if ($cek_sdh_ujian < 1)	{		
					...
					foreach ($mapels as $mapel) {
						$q_soal			= $this->db->query("SELECT id, file, tipe_file, soal, opsi_a, opsi_b, opsi_c, opsi_d, opsi_e, '' AS jawaban, jml_opsi FROM m_soal WHERE id_mapel = '".$mapel['mapel_id']."' AND id_guru = '".$cek_detil_tes->id_guru."' ".$acakan." LIMIT ".$mapel['jml_soal'])->result();
							$jumlah = 0;
							foreach ($q_soal as $s) {
								$jumlah++;
								...
								$soal_per->jml_opsi = $s->jml_opsi;
								$soal_urut_ok[$i] = $soal_per;
								$i++;
							}
							$data["soal"][] = count($q_soal);
							$data["waktu"][] = $mapel['waktu'];
					...
					$waktu_selesai  = tambah_jam_sql($waktu);
					...
					$soal_urut_ok	= array();
					$current_mapel_id = $old_mapel_id = 0;
					for ($i = 0; $i < sizeof($urut_soal); $i++) {
						...
						$current_mapel_id = $ambil_soal->id_mapel;
						if($current_mapel_id != $old_mapel_id){
							$mapel = $this->db->query("SELECT * FROM mapel_tes WHERE mapel_id = $current_mapel_id AND guru_tes_id = $uri4")->row_array();
							$old_mapel_id = $current_mapel_id;
							$data['soal'][] = $mapel['jml_soal'];
							$data['waktu'][] = $mapel['waktu'];
						}
					}
				...
				$a['html'] = $html;
				$a['data'] = '
					<input type="hidden" name="jml_soal_per_mapel" value="'.implode(',', $data['soal']).'">
					<input type="hidden" name="jml_waktu_per_mapel" value="'.implode(',', $data['waktu']).'">
				';
