...
<script type="text/javascript">
    ...
    var array_soal = (form.jml_soal_per_mapel).split(",");
    var time = 0;
    var index_time = 0;
    var soal_ke = 0;
    ...
    simpan_sementara = function() {
        ...
        for (var i = 1; i < jml_soal; i++) {
            ...
            if (jawab != undefined) {
                if (ragu == "Y") {
                    if (jawab == "-") {
                        hasil_jawaban += '<a id="btn_soal_'+(i)+'" class="btn btn-default btn_soal btn-sm soal_sesi_'+(index)+'" onclick="return buka('+(i)+');">'+(i)+". "+jawab+"</a>";
                    } else {
                        hasil_jawaban += '<a id="btn_soal_'+(i)+'" class="btn btn-warning btn_soal btn-sm soal_sesi_'+(index)+'" onclick="return buka('+(i)+');">'+(i)+". "+jawab+"</a>";
                    }
                } else {
                    if (jawab == "-") {
                        hasil_jawaban += '<a id="btn_soal_'+(i)+'" class="btn btn-default btn_soal btn-sm soal_sesi_'+(index)+'" onclick="return buka('+(i)+');">'+(i)+". "+jawab+"</a>";
                    } else {
                        hasil_jawaban += '<a id="btn_soal_'+(i)+'" class="btn btn-success btn_soal btn-sm soal_sesi_'+(index)+'" onclick="return buka('+(i)+');">'+(i)+". "+jawab+"</a>";
                    }
                }
            } else {
                hasil_jawaban += '<a id="btn_soal_'+(i)+'" class="btn btn-default btn_soal btn-sm soal_sesi_'+(index)+'" onclick="return buka('+(i)+');">'+(i)+". -</a>";
            }
            if(array_soal[index] == i){
                index++;
            }
        }
        ...
        $.ajax({    
            ...
            for (var i = 0; i < panjang; i++) {
                if (response.data[i] != "_N") {
                    ...
                    if (pc_getjwb[1] == "Y") {
                        if (pc_getjwb[0] == "-") {
                            hasil_jawaban += '<a id="btn_soal_'+(i+1)+'" class="btn btn-default btn_soal btn-sm soal_sesi_'+(index)+'" onclick="return buka('+(i+1)+');">'+(i+1)+". "+pc_getjwb[0]+"</a>";
                        } else {
                            hasil_jawaban += '<a id="btn_soal_'+(i+1)+'" class="btn btn-warning btn_soal btn-sm soal_sesi_'+(index)+'" onclick="return buka('+(i+1)+');">'+(i+1)+". "+pc_getjwb[0]+"</a>";
                        }
                    } else {
                        if (pc_getjwb[0] == "-") {
                            hasil_jawaban += '<a id="btn_soal_'+(i+1)+'" class="btn btn-default btn_soal btn-sm soal_sesi_'+(index)+'" onclick="return buka('+(i+1)+');">'+(i+1)+". "+pc_getjwb[0]+"</a>";
                        } else {
                            hasil_jawaban += '<a id="btn_soal_'+(i+1)+'" class="btn btn-success btn_soal btn-sm soal_sesi_'+(index)+'" onclick="return buka('+(i+1)+');">'+(i+1)+". "+pc_getjwb[0]+"</a>";
                        }
                    }
                } else {
                    hasil_jawaban += '<a id="btn_soal_'+(i+1)+'" class="btn btn-default btn_soal btn-sm soal_sesi_'+(index)+'" onclick="return buka('+(i+1)+');">'+(i+1)+". -</a>";
                }
                if((array_soal[index]-1) == i){
                    index++;
                }
            }
    ...
    format_date = function(date_){
        var year = date_.getFullYear();
        var month = ("0" + (date_.getMonth() + 1)).slice(-2);
        var date = ("0" + date_.getDate()).slice(-2);
        var hours = ("0" + date_.getHours()).slice(-2);
        var minutes = ("0" + date_.getMinutes()).slice(-2);
        var seconds = ("0" + date_.getSeconds()).slice(-2);
        return year + "-" + month + "-" + date + " " + hours + ":" + minutes + ":" + seconds;
    }
    
    hitung = function(index, tgl_mulaia) {
        var jam_mulai = '<?php echo $jam_mulai; ?>';
        var tgl_mulai = '<?php echo date('Y-m-d H:i:s'); ?>';
        var tgl_selesai = '<?php echo $jam_selesai; ?>';
        var f_asal  = $("#_form");
        var form  = getFormData(f_asal);

        var array_waktu = (form.jml_waktu_per_mapel).split(",");

        time = time + parseInt(array_waktu[index]);
        var timestamp = new Date(jam_mulai).getTime() + time*60000;
        var tgl_selesai1 = new Date(timestamp);
        var tgl_selesai = format_date(tgl_selesai1);

        var timestamp_jam_mulai = new Date(jam_mulai).getTime();
        var timestamp_jam_sekarang = new Date(tgl_mulai).getTime();
        
        var time_elapsed = timestamp_jam_sekarang - timestamp_jam_mulai;
        durasi = durasi + parseInt(array_waktu[index]);

        var temp_hours = durasi * 60000 - time_elapsed;
        var temp_minutes = temp_hours/60000;
        var temp_seconds = (temp_hours%60000)/1000;

        if(temp_minutes < 60){
            var hours = 0;
            var minutes = temp_minutes;
            var seconds = temp_seconds;
        } else {
            var hours = temp_minutes/60;
        ...
        function timer(){
            ...
            $("div#clock").html(
                `${hours}:${parseInt(minutes)}:${seconds}`
            )
            ...
            if( seconds < 0 ){
                ...
                if( minutes < 0 ){
                    ...
                    if( hours < 0 ){
                        clearInterval();
                        if(index_time != (array_waktu.length-1)){
                            soal_ke = soal_ke + parseInt(array_soal[index_time]);
                            buka( soal_ke + 1 )
                            index_time++;
                            hitung(index_time, tgl_selesai);
                            reloadPage();
                        } else {
                            simpan_akhir();
                        }
