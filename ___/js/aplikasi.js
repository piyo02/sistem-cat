...
function m_ujian_s() {
	...
	window.location.href(base_url+"adm/m_ujian/edit/0"); 
	// $.ajax({		
	// 	type: "GET",
	// 	url: base_url+"adm/m_ujian/edit/0",
...
//jumlah opsi
$(function(){
	var dropdown_jml_opsi = $("#jml_opsi");
	var fgsoal = $(".fgsoal");
	var pilihan_e = fgsoal[8];
	var option = $("#jawaban");
console.log(dropdown_jml_opsi.val())
	if(dropdown_jml_opsi.val() == 4){
		pilihan_e.hidden = true;
		option.find('option[value=E]').remove();
	}

	dropdown_jml_opsi.change(function(){
		var jml_opsi = this.value;
		if(jml_opsi == 4){
			pilihan_e.hidden = true;
			option.find('option[value=E]').remove();
		}
		else if ( jml_opsi == 5 ){
			pilihan_e.hidden = false;
			option.append('<option value="E">E</option>')
		}
	});
})
